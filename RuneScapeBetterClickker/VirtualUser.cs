...
namespace Click
{
    ...
    public class VirtualUser
    {
        ...
        [DllImport("user32.dll")]
        internal static extern uint SendInput(uint nInputs, [MarshalAs(UnmanagedType.LPArray)] [In] ref INPUT pInputs,
            int cbSize);
        ...
        public void AddPointToSeries(Point p)
        {
            ...
            clickPoints?.Add(new ClickPoints {Point = p, Time = DateTime.Now.ToFileTimeUtc()});
                //TODO voeg de tijd toe, deze is relatief aan de laatste click voor deze click
            points?.Add(p);
        }
        ...
        public void ExecuteSeries()
        {
            foreach (var point in points)
            {
                ...
                var sleep = 100 - r.Next(20);
                Thread.Sleep(sleep);
            }
        }

        // ReSharper disable once RedundantAssignment
        public void ExecuteSeries(ref bool isPlaying, bool useTime = false)
        {
            isPlaying = true;
            ...
            for (var i = 0; i < clickPoints.Count; i++)
            {
                var cp = clickPoints[i];
                if (!isPlaying)
                    break;
                ...
                if (useTime)
                {
                    wait = clickPoints.Count < i ? cp.Time - clickPoints[i + 1].Time : 0;
        ...
        public void NewClickLeftMouse(int x, int y)
        {
            ...
            var structInput = new INPUT {type = InputType.MOUSE};
